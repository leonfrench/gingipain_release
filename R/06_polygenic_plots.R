...
ggsave(filename = here('results', 'microarray', 'HBA-SRP_CHAT_enrichment.pdf'), width=10, height=7, dpi=300)
#for PDF use 10x7
...
ggsave(filename = here('results', 'microarray', 'HBA-SRP_CHAT_enrichment_hypothalamus.pdf'), width=10, height=7, dpi=300)

#########################################################################################################
# Plot SRP AUC vs AVP expression
#########################################################################################################
labels <- chol_labels
ggplot(enrichment_table, aes(y=srp_AUC, x=AVP, label=brain_structure)) +
  geom_point(data = enrichment_table %>% filter(!str_detect(brain_structure, paste(labels, collapse='|'))), color= 'darkgrey') +
  geom_point(data = enrichment_table %>% filter(str_detect(brain_structure, paste(labels, collapse='|'))), color= 'red') +
  geom_text_repel(data = enrichment_table %>% filter(str_detect(brain_structure, paste(labels, collapse='|'))),
                  #force = 10,
...
ggsave(filename = here('results', 'microarray', 'HBA-SRP_AVP_enrichment.pdf'), width=10, height=7, dpi=300)
#for PDF use 10x7
...
ggsave(filename = here('results', 'microarray', 'HBA-SRP_AVP_enrichment_hypothalamus.pdf'), width=10, height=7, dpi=300)
#for PDF use 10x7
